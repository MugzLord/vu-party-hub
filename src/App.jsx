  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-200 font-sans">
      <header className="bg-[#111827] border-b border-white/5 p-4 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-2"><div className="w-8 h-8 bg-indigo-600/10 rounded-lg flex items-center justify-center border border-indigo-500/30"><CalendarDays size={18} className="text-indigo-500"/></div><h1 className="font-black text-white text-base">VU HUB</h1></div>
        {currentUser ? (
            <div className="flex items-center gap-4">
                <span className="text-[10px] bg-white/5 px-3 py-1 rounded-full uppercase font-bold text-indigo-400">{currentUser.username}</span>
                <button onClick={()=>{setCurrentUser(null); localStorage.removeItem(SESSION_KEY);}} className="text-[10px] font-black uppercase text-rose-500"><LogOut size={14}/></button>
            </div>
        ) : <button onClick={() => setShowAuthGate(true)} className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl text-[10px] font-black uppercase"><Shield size={14}/> Admin Login</button>}
      </header>
      ...
      <div className="flex p-4 gap-2 bg-[#111827]/50 border-b border-white/5 overflow-x-auto">
        {['Guide', 'Monthly', isStaff ? 'Manage' : ''].filter(Boolean).map(t => (
            <button key={t} onClick={()=>setView(t)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 ${view===t ? 'bg-indigo-600 text-white' : 'bg-[#1f2937] text-slate-400 hover:bg-slate-700'}`}>
                {t === 'Guide' && <BookOpen size={12}/>}{t === 'Monthly' && <Calendar size={12}/>}{t === 'Manage' && <Edit size={12}/>}{t}
            </button>
        ))}
      </div>
